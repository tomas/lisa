...
deploy_tasks.pull_changes = function(env) {
  ...
  function fetch() {
    return 'git fetch ' + remote + ' ' + [branch, branch].join(':') + ' --force';
  }
  ...
  if (env.releases_path) {
    ...
    var str = [fetch(), reset([remote, branch].join('/'))].join(' && '); // reset to origin/master
...
deploy_tasks.install_dependencies = function(env) {
  var cmd  = '([ -f package.json ] && npm install --production || exit 44)';
  cmd += ' || ([ -f Gemfile.lock ] && ' + bundle_command(env) + ' || exit 44)';
  cmd += ' || [ $? -ne 44 ] && echo "No dependencies to install."';
