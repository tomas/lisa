...
var build_sequence = function(task_names, stage) {
  ...
  function task_in_roles(task) {
    if (typeof task == 'string')
      return { all: in_current_path(stage.env, task) };

    var obj = {};
    for (var role in task) {
      obj[role] = in_current_path(stage.env, task[role]);
    }
...
tasks.show_current_commit = function(env) {
  var cmd = 'echo Last commit: $(git --no-pager log --format="%aN (%h):%n> %s" -n 1)';
  return in_destination_path(env, cmd);
}

tasks.symlink_shared_paths = function(env) {

  if (env.shared_paths.length == 0) {
