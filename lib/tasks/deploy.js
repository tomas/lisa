...
deploy_tasks.pull_changes = function(env) {
  ...
  function show_commit(label, store_as) {
    var cmd = 'head_commit=$(git --no-pager log ' + branch + ' --format="%aN (%h) -> %s" -n 1); echo ' + label + ' commit: $head_commit';
...
deploy_tasks.install_dependencies = function(env) {
  var cmd  = '([ ! -f package.json ] && exit 0 || npm install --production)';
  cmd += ' && ([ ! -f Gemfile.lock ] && exit 0 || ' + bundle_command(env) + ')';
...
deploy_tasks.precompile_assets = function(env) {
  return in_destination_path(env, 'bundle exec rake assets:precompile RAILS_GROUPS=assets RAILS_ENV={{environment}}');
}
