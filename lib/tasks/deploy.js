...
deploy_tasks.pull_changes = function(env) {
  ...
  if (env.releases_path) {
    ...
    if (env.deploy_tag) {
      cmds.push(get_tag() + ' && ' + reset('$latest_tag'));
    } else {
      cmds.push(reset('HEAD')); // reset to HEAD, then clone the branch we want
      cmds.push(in_build_path(env, 'git clone ' + env.repo_path + ' . --recursive --branch ' + branch));
    }

  } else {
    var str = [fetch(), reset(local)].join(' && '); // reset to origin/master
