...
deploy_tasks.pull_changes = function(env) {
  ...
  function sync_submodules() {
    // return 'git submodule sync --recursive && git submodule update --init --checkout --recursive'
    return 'git submodule update --init --checkout --recursive'
  }
  ...
  if (env.releases_path) {
    ...
    // if (env.has_submodules) cmds.push(in_build_path(env, sync_submodules()));
    ...
    if (env.has_submodules) str += sync_submodules();
...
deploy_tasks.db_migrate = function(env) {
  return in_destination_path(env, 'bundle exec rake db:migrate RAILS_ENV={{environment}} RACK_ENV={{environment}}');
}
