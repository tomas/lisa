...
var build_sequence = function(task_names, stage) {
  ...
  function append(task_name) {
    ...
    for (var name in stage.roles) {
      ...
      if (custom_tasks[task_name]) {
        list[task_name][name] = in_current_path(stage.env, custom_tasks[task_name]);
...
tasks.write_lock_file = function(env) {
  return check_command('[ ! -f ' + env.lock_file_path + ' ] && touch ' + env.lock_file_path, 'Unable to write lock file.');
}
...
tasks.pull_changes = function(env) {
  ...
  if (env.releases_path) {
    ...
    cmds.push('cd ' + env.repo_path + ' && git fetch ' + remote + ' ' + branch + ' --force');
    cmds.push(in_build_path(env, 'git clone ' + env.repo_path + ' . --recursive --branch ' + branch));
    return cmds.join(' && ');
  } else {
    return in_release_path(env, 'git pull ' + remote + ' ' + branch + ' --force');
  }

}

tasks.show_current_commit = function(env) {
  var branch = env.branch || 'master',
      cmd = 'echo Last commit: $(git --no-pager log ' + branch + ' --format="%aN (%h):%n> %s" -n 1)';
...
exports.prepare = function(stage, args) {
  ...
  var cmds = build_sequence(deploy_sequence, stage);
  // console.log(cmds);
