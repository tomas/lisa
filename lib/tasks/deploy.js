...
var build_sequence = function(task_names, options) {
  var list = [],
      custom_tasks = options.tasks.deploy || {};
    
  function append(command, name) {
    var str  = new Buffer(command);
    str.desc = name;
    list.push(str); 
  }
  
  task_names.forEach(function(task) {
    if (custom_tasks['before_' + task])
      append(custom_tasks['before_' + task], task)
    
    if (custom_tasks[task])
      append(custom_tasks[task], task)
    else
      append(tasks[task](options.env), task);

    if (custom_tasks['after_' + task])
      append(custom_tasks['after_' + task], task)
  })
...
tasks.set_environment = function(env) {
  var vars = [];
  for (var key in env) {
    vars.push(key + '="' + env[key] + '"');
  }

  return 'export ' + vars.join(' ');
}
...
exports.prepare = function(stage, args) {
  ...
  return {
    sequence: build_sequence(deploy_sequence, stage),
    rollback: build_sequence(rollback_sequence, stage)
  }
}
