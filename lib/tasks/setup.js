...
function clone_repo(dir, repo, has_releases) {
  var bare = has_releases ? '--bare ' : '';
  return '[ -d ' + dir + ' ] && echo "Repo found." || git clone ' + bare + repo + ' ' + dir.replace('.git', '');
}
...
exports.prepare = function(stage, args) {
  ...
  cmds.clone_repo = { all: clone_repo(stage.env.repo_path, stage.env.repository, !!stage.env.releases_path) };
  return cmds;
}
