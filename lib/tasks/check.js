...
checkers.process = function(env, pidfile) {
  ...
  return '[ -f ' + file + ' ] && pid=$(cat ' + file + ' 2> /dev/null) && (ps -p $pid > /dev/null && mem=$(ps -p $pid uh 2> /dev/null | cut -d" " -f12) && echo "Process running, using $mem bytes of mem." || echo "HOLY CRAP. PID $pid is not running!") || (echo "SHOOT! Pidfile not found: ' + pidfile + '" && false)';
}
...
exports.description = "Runs a quick check of the status of your servers and app."
