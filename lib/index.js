...
function load_task(stage, task, args) {
  ...
  try {
    ...
    abort('Failed loading ' + task + ' task: ' + e.message);
  }
  
  var commands = task.prepare(stage, args);

  if (subtask) {
    if (!commands[subtask]) abort('Subtask not found: ' + subtask);
...
function load(args) {
  var config_file = args.config || defaults.config_file,
      dir = process.cwd(),
      file_path = join(dir, config_file);

  if (fs.existsSync(file_path)) 
    return require(file_path);

  abort(config_file + ' not found in ' + dir);
}
