...
var DEFAULT_BRANCH_NAME = 'master'; // 'main'
...
function prompt_user(message, cb) {
  var rl, readline = require('readline');

  function open_prompt() {
    if (!rl) rl = readline.createInterface(process.stdin, process.stdout);
    else process.stdin.resume(); // interface exists
    return rl;
  }
  ...
  rl = open_prompt();
  rl.question(message, function(reply) {
    close_prompt();
    cb(reply)
  });
}

function get_current_branch(cb) {
  exec('git rev-parse --abbrev-ref HEAD', function(err, stdout) {
    cb(stdout.toString().trim())
  });
}

function branch_exists(branch_name, cb) {
  exec('git ls-remote --heads origin ' + branch_name, function(err, stdout) {
    cb(stdout.toString().trim() == '')
  });
}

function get_branch(command, config_branch, cb) {
  if (config_branch)
    return cb(config_branch);
  else if (command != 'deploy')
    return cb(DEFAULT_BRANCH_NAME);

  get_current_branch(function(br) {
    prompt_user('Branch to deploy [' + br + ']: ', function(answer) {
      cb(answer || br);
    })
  });
}
...
exports.set = function(command, config, stage, dir, cb) {
  ...
  env.no_binstubs   = config.bundle_binstubs == false;
  // env.has_submodules = config.has_submodules || existsSync(join(dir, '.gitmodules'));
  ...
  get_branch(command, env.branch || config.branch, function(branch) {
    env.branch = branch;
    cb({ env: env, roles: roles, tasks: root.tasks });
  })
}
