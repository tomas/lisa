...
var stage_settings = ['environment', 'branch', 'deploy_to', 'primary_host'];
...
exports.set = function(config, stage) {
  ...
  if (stage) {
    ...
    for (var key in stage_settings) {
      if (config.stages[stage][stage_settings[key]]) {
        env[stage_settings[key]] = config.stages[stage][stage_settings[key]];
      }
    }
