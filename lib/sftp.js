...
function startSFTP(stage, cb, mkdir_path) {
  ...
  connect.one(host, role, function(err, conn) {
    ...
    if (mkdir_path) {
      conn.exec('mkdir -p "' + mkdir_path + '"', function(err, stream) {
        if (err) throw err;
        stream.on('exit', open_sftp);
      })
    } else {
      open_sftp();
    }
...
exports.put = function(stage, local, remote, cb) {
