...
function write(label){
  ...
  var thisLine = index;
  index += 1;
  var line = " [ " + placeholder + " ] " + label;

  var update = function(status, opts) {
    var delta = placeholder.length - status.length;
    var delta1 = Math.floor(delta / 2);
    var text = label;
    // if (opts && opts.result_text) text += ' -- ' + result_text.trim();
    ...
    if (opts && opts.color) {
      terminal.write(" [ ").color(opts.color).write(status).reset().write(" ] " + text);
    } else {
      terminal.write(" [ " + status + " ] " + text);
    }
  ...
  var returnObj = function(message, opts) {
    update(message, opts);
  }
  ...
  returnObj.stop = function(status, opts) {
    clearInterval(this.timer);
    if (status || opts) update(status, opts);
  }
