...
function prepare(name, command, env) {
  ...
  for (var key in env) {
    var value = env[key];
    if (value) str = str.replace(new RegExp('{{' + key + '}}', 'g'), value);
  }

  str = str.replace(/(^|\s)bundle /g, '$1' + bundle_paths + ' bundle ')

  var obj = Buffer.from(str);
  obj.desc = name;
  return obj;
}
...
function close_handles() {
  ...
  handles.forEach(function(obj) {
    if (typeof obj.end == 'function') {
      if (obj != process.stdout && obj != process.stderr)
        try { obj.end(); } catch(e) { }
    }
  })
}
...
function Runner(stage, commands, opts) {
  ...
  this.silent   = process.env.SILENT;
...
Runner.prototype.set = function(commands) {
  ...
  var time = new Date();
  output.notice('\n ----- lisa running ' + this.commands_count + ' commands on ' + Object.keys(this.stage.roles).length + ' roles at ' + time + '.\n');
  ...
  function initialize(role) {
    ...
    connect.many(roles[role], role, function(err, group) {
      ...
      group.on('stdout', function(server, chunk, command) {
        ...
        if (str != '') server[type]((command.desc || command) + ': ' + str);
      })
...
Runner.prototype.update_progress = function(prev_success, result) {
  if (this.stopped || this.silent)
    return;
  ...
  if (typeof prev_success != 'undefined') {
    ...
    if (previous) {
      var lastLine = String(result.stdout || '').trim().split('\n').pop();
      previous.stop(mark, { result_text: lastLine });
    }
  }
...
Runner.prototype.next = function() {
  ...
  function all_done(res) {
    self.update_progress(true, res); // success
    self.next();
  }

  // run when once group finishes
  function group_done(err, res) {
    self.emit('command', err);

    if (err) { // show error on progress bar
      self.update_progress(false, res);
      return self.stop(err);
    }

    errors.push(err);
    --counter || (!self.stopped && all_done(res));
  }
