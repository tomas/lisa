...
/*
function CommandSet(list) {
  this.list  = list;
  this.count = list.length;
  this.index = 0;
}

CommandSet.prototype.show = function() {
  this.menu = [];
...
function Runner(stage, commands) {
  ...
  this.status   = 'paused';
  this.groups   = [];
  this.verbose  = process.env.FAKE || process.env.VERBOSE;
...
Runner.prototype.open = function(cb) {
  ...
  function initialize(role) {
    ...
    if (self.verbose)
      output.status('\n ----- [' + role + '] Connecting to: ' + roles[role].hosts.join(', '));
...
Runner.prototype.next = function() {
  ...
  function done(err) {
    ...
    --counter || (!self.stopped && self.next(err));
  }
